...
// الصفحة المعروضة تحمل الادّعاء فعلاً (ملاحظة Codex). وصورةُ Markdown
// ("![نصٌّ بديلٌ](عنوان)") حالةٌ خاصّةٌ يجب معالجتها *قبل* رابط Markdown
// العاديّ ("[نصٌّ](عنوان)"): النمطُ العاديُّ يطابق الجزء بعد "!" فقط
// ("[نصٌّ](عنوان)")، تاركاً "!" حرفيّةً معلَّقةً في الناتج (كـ"!approved"
// بدل "approved" — ملاحظة Codex)؛ نصُّها البديلُ يظهر فعليّاً لقارئات
// الشاشة وحين يتعذّر تحميل الصورة، فادّعاءٌ فيه ادّعاءٌ حقيقيٌّ. ورابطُ
// Markdown "[نصّ](عنوان)" يُستبدَل بنصّه وحده (لا بالعنوان): عبارةٌ كـ
// "معتمد [سريريّاً](...)" تُعرَض "معتمد سريريّاً" فعليّاً، لا أن يبقى بين
...
// النصّ بعينه) تُعرَض نصَّها أيضاً بمعيار Markdown — بخلاف الصيغتَين أعلاه،
// هذه لا تُسقَط دائماً: قوسان حول نصٍّ عاديٍّ (لا رابط) شائعان في نثرٍ
// عاديٍّ (كتعليقٍ بين قوسين)، فتُقارَن فقط بمجموعة المعرّفات الموجودة فعلاً
// في الملفّ (سطرٌ بصيغة "[معرّف]: عنوان")، لا كلُّ "[نصّ]" حرفيّاً (ملاحظة
// Codex الثالثة). دالّةٌ مشتركةٌ لا مكرّرةٌ في normalize() وحدها: فحوصُ
// البيانات الحسّاسة تحتاج حلَّ الروابط أيضاً — رقمٌ كـ"+49 [151](#)
// 23456789" يُعرَض رقماً عاديّاً فعليّاً (ملاحظة Codex الرابعة).

...
// [\t\n\f\r ] لا "\s" هنا أيضاً (أسوةً بحدّ اسم الوسم في META_TAG_RE أدناه):
// صنفُ جافاسكربت \s يونيكوديٌّ (يشمل U+00A0 اللامسافةَ الفاصلة)، لكنّ محلّل
// سماتِ HTML5 لا يفصل بين السمات إلا بخمسة محارف مسافةٍ بيضاءَ ASCII. وسمٌ
// مثل <meta name="robots" content="noindex"> ليس فيه سمةُ content
// أصلاً بمعيار HTML5 (اللامسافة تلتحم باسم السمة التالية بدل الفصل بينهما)،
// فقبولُها فاصلاً هنا كان يستخرج content="noindex" خطأً ويُفشل الفحصَ
// لصفحةٍ قابلةٍ للفهرسة فعلاً (ملاحظة Codex).
const HTML_ASCII_WS = "\\t\\n\\f\\r ";
const ATTR_RE = new RegExp(
  `([^${HTML_ASCII_WS}"'=<>\`/]+)(?:[${HTML_ASCII_WS}]*=[${HTML_ASCII_WS}]*(?:"([^"]*)"|'([^']*)'|([^${HTML_ASCII_WS}"'=<>\`]+)))?`,
  "g"
);
...
// (?=[\t\n\f\r />]) لا "\s": صنفُ جافاسكربت \s يونيكوديٌّ (يشمل مثلاً
// U+00A0 اللامسافة الفاصلة)، لكنّ محلّل HTML5 لا يعتبر إلا خمسة محارف
// مسافةً بيضاءَ فاصلةً لاسم الوسم (Tab وLF وFF وCR والمسافة العاديّة)؛
// "<metaU+00A0name=..." الحقيقيّ ليس عنصرَ meta أصلاً بمعيار HTML5 (اللامسافة
// جزءٌ من اسم الوسم لا فاصلٌ عنه)، فقبولُها هنا حدّاً كان يُطابقه خطأً
// كأنّه توجيه robots فعليّ ويُفشل الفحصَ لصفحةٍ قابلةٍ للفهرسة فعلاً (ملاحظة
// Codex). حدّ اسم الوسم هنا يشترط تحديداً أن يلي "meta" مسافةً بيضاءَ
// ASCII أو "/" أو ">" — لا أيّ محرفٍ يونيكوديٍّ يُعَدّ "مسافةً بيضاء" بمعنى
// جافاسكربت العامّ.
// فرعُ الاحتياط [^>] يستبعد أيضاً "\"" و"'" (لا ">" فقط): بدونهما يبقى أيّ
// محرف اقتباسٍ مقبولاً عبر بديلَين معاً (بديل الاقتباس نفسه، وبديل الاحتياط
// غير المستبعِد له)، فوسمٌ غيرُ مغلَقٍ يحوي اقتباساتٍ كثيرةً بلا ">" يفتح
// عدداً أُسّيّاً من توزيعاتٍ ممكنةٍ بين البديلَين قبل أن يفشل — تراجعٌ عكسيّ
// (catastrophic backtracking) فعليٌّ يستنزف وقتَ CI حتى انتهاء المهلة على
// أيّ index.html ينتهي بـ"<meta " متبوعةً بعشرات التكرارات لـ'"a' بلا ">"
// مغلقة (ملاحظة Codex؛ نفسُ الإصلاح طُبِّق على TAG_GAP المكافئة في
// check-no-approval-claims.js). استبعادُ الاقتباس من فرع الاحتياط يجعل
// البديلَين متنافيَين، فلا توزيعاتٍ بديلةٌ للتراجع عبرها؛ لا يغيّر هذا أيَّ
// وسمٍ صالحٍ يُطابَق فعليّاً، إذ الاقتباس داخل وسمٍ سليمٍ يقع دائماً ضمن
// قيمة سمةٍ مقتبسة.
const META_TAG_RE = /<meta(?=[\t\n\f\r />])(?:"[^"]*"|'[^']*'|[^>"'])*>/gi;

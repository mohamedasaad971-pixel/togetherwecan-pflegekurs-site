...
// يُسقَط تعليقُ HTML (<!-- ... --> ) ومحتوى <script>/<style> قبل البحث عن
// <meta>: أيٌّ منها خاملٌ فعليّاً (لا ينفّذه المتصفّح كتوجيه HTML) — كنصّ
// JSON-LD أو مثالٍ داخل سكربتٍ يحتوي حرفيّاً على "<meta ...noindex...>"،
// فمطابقتُه كأنّه توجيهٌ فعليٌّ تمنع فهرسة صفحةٍ تبقى قابلةً للفهرسة فعلاً
// (ملاحظة Codex؛ التعليقات كانت أوّل جولة، والسكربت/الأسلوب جولةٌ لاحقة).
const html = htmlRaw
  .replace(/<!--[\s\S]*?-->/g, "")
